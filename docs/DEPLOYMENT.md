...
|-----|----------------|---------------|
| **Claim allocation binding** | `claimMint` / `claimRedeem` require proof amount = on-chain Phase-2 allocation (`batchMintKashAllocation` / `batchRedeemNetAsset`) | Evil Merkle roots cannot pay arbitrary amounts. Proofs must match settlement math the bot already uses. Residual: allocations still derive from bot-reported `G` in `markBatchOpsDone` (pro-rata; Phase-2 balance check). Accepted for beta. |
| **NAV deviation cap** | `updateNAV` limited to **±15%** per write vs current NAV (`NAV_MAX_DEVIATION_BPS = 1500`). Bound applies from genesis (`currentNAV = 1e18`; no `navInitialized` hatch). | Jumps >15% **revert on-chain**. A full batch still writes NAV 2–3 times; the bot already stops hops >3%. Large recovery needs **stepped updates**, not one giant write. Do not reset dust NAV to $1.00 if that move exceeds 15%. |
| **Oracle staleness** | `getEthPrice` / `getBtcPrice` revert on `updatedAt == 0`, `answeredInRound < roundId`, or `updatedAt` older than **25 hours** | Before deploy, confirm feed heartbeats on [data.chain.link](https://data.chain.link/) (Arbitrum ETH/USD + BTC/USD are ~24h). If you substitute a faster feed, tighten the constant to heartbeat + ~1h buffer. Monitor `StalePrice` so a dead feed is not mistaken for a stuck bot. |
