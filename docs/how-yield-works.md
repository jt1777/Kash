...
The contract does not automatically calculate NAV from the portfolio — the operator calculates portfolio value externally and submits it each day. On-chain price feeds are used to value deposits at batch time, but the complete portfolio valuation (collateral positions, perp position values, accrued funding) is computed by the operator and then written to the contract. All NAV submissions are recorded on-chain and publicly verifiable. For step-by-step instructions on checking NAV yourself, see [Verify NAV](verify-NAV.md).
