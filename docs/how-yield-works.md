...
The contract does not automatically calculate NAV from the portfolio — `updateNAV` is a value the operator calculates externally and submits each day. On-chain price feeds are used to value deposits at batch time, but the complete portfolio valuation (collateral positions, perp position values, accrued funding) is computed by the operator and then written to the contract. All NAV submissions are recorded on-chain and publicly verifiable.

> **Roadmap — automated on-chain NAV:** All portfolio balances (lending protocol collateral, borrowed stablecoin, perp DEX spot balance and position value) are readable from on-chain contracts in the current deployment. A planned upgrade will add an on-chain `calculateNAV()` function that reads these directly, combined with automated daily execution via Chainlink Automation. This will remove the need for any manual NAV submission and eliminate the operator trust assumption entirely. On mainnet with a cross-chain perp exchange, a compatible cross-chain data solution would also be required for the perp side.
