...
NAV is updated at least **once per day** during batch processing (around 23:40–23:59 UTC). The operator calculates the true portfolio value off-chain, then submits the new NAV to the contract before distributing tokens. This means the NAV displayed in the app reflects the previous day's closing value.

The contract does not automatically calculate NAV from the portfolio — the operator calculates portfolio value externally and submits it each day. On-chain price feeds are used to value deposits at batch time, but the complete portfolio valuation (collateral positions, perp position values, accrued funding) is computed by the operator and then written to the contract. All NAV submissions are recorded on-chain and publicly verifiable. For step-by-step instructions on checking NAV yourself (Aster vaults on Arbitrum), see [Verify NAV](verify-NAV.md).
...
|-----------|--------------|
| 00:00 – 23:39 | User window open: deposits and redemptions may be submitted |
