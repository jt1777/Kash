...
export async function executeRedeemTailDeltaPipeline(
  ...
  switch (tail) {
    ...
    case 'falling':
      ...
      break;
    case 'rising':
      // Owner USDC: HL withdraw fee once (tailCoverHlFeeShortfall, ≤~$1.1) only.
      // withdraw→11a→repay loop uses strategy Aave wBTC only (no owner USDC in the loop).
      // If wBTC < G after aave_withdraw_rest: redeem-gap cover + 11b (separate from Aave loop).
      phases = [
        { deltaId: 'Δtail cover_hl_fee_shortfall', step: tailCoverHlFeeShortfall },
...
/**
 * Rising tail: iterative withdraw → 11a swap → repay until strategy Aave debt is cleared.
 * Uses strategy Aave collateral only — no owner USDC in this loop (HL fee is pre-covered once).
 * Payout collateral is withdrawn afterward by `aave_withdraw_rest` once debt is zero.
 */
async function executeRisingWithdrawSwapRepayLoop(ctx: OpsContext): Promise<void> {
  ...
  for (let round = 1; round <= maxRounds; round++) {
    const debt = strategyAaveDebtToRepay(fresh);
    if (debt === 0n) {
      if (round > 1) {
    ...
    // Vault USDC already covers strategy debt — repay without withdrawing collateral.
    if (sf === 0n) {
      await executeAaveRepay(fresh);
      fresh = await refreshOpsCtx(fresh);
      if (strategyAaveDebtToRepay(fresh) === 0n) {
        console.log('         ↪ rising withdraw-swap-repay loop complete (repay from contract USDC)');
        return;
      }
      continue;
    }
  ...
  const debtFinal = strategyAaveDebtToRepay(fresh);
  const sfFinal = usdcShortfallVsContract(fresh);
  const skipSwapFinal = await shouldSkipRising11aSwap(fresh, sfFinal);
  if (debtFinal > 0n && sfFinal > 0n && !skipSwapFinal) {
    throw new Error(
      `Rising tail: withdraw-swap-repay loop exhausted ${maxRounds} rounds ` +
        `(strategy debt ${fmtUsdc(debtFinal)}, USDC shortfall ${fmtUsdc(sfFinal)}). Re-run or raise RISING_11A_LOOP_MAX_ROUNDS.`,
    );
  }
}
 ...
 * Size per round: min(95% × maxSafeWithdraw, USDC-shortfall in asset). Redeem payout uses aave_withdraw_rest.
 */
const aaveWithdrawPartial: OpStep = {
  ...
  describe: (ctx) => {
    ...
    return (
      ...
      `strategy debt ${fmtUsdc(debt)}, swap chunks ~${fmtAsset(assetForSf, ctx)} (payout via aave_withdraw_rest after debt cleared)`
    );
  },
  canSkip: async (ctx) => {
    ...
    if (!(await shouldSkipRising11aSwap(ctx, sf))) {
      const spotDex = await ctx.kashYield.spotDexAddress().catch(() => null);
      if (!spotDex || spotDex === ethers.ZeroAddress) {
...
/**
 * HL L1→L2 withdraw fee (~$1): release at most HL_WITHDRAW_FEE_TOLERANCE_USDC from ownerUsdcReserve
 * once per tail, before Aave repay / rising loop. Larger debt shortfalls are handled in the loop.
 */
async function executeTailCoverHlFeeShortfall(ctx: OpsContext): Promise<void> {
  const sf = usdcShortfallVsContract(ctx);
  if (sf === 0n) return;
  ...
  const feeSlice = sf < cap ? sf : cap;
  const amount = feeSlice < reserve ? feeSlice : reserve;
  if (amount === 0n) return;
  console.log(
    `         coverUsdcShortfall ${fmtUsdc(amount)} (HL fee once, slice≤${fmtUsdc(cap)}, ` +
      `debt shortfall ${fmtUsdc(sf)}, ownerUsdcReserve ${fmtUsdc(reserve)})`,
  );
...
const tailCoverHlFeeShortfall: OpStep = {
  ...
  describe: (ctx) => {
    ...
    const cap = getHlWithdrawFeeToleranceUsdc6();
    const feeSlice = sf < cap ? sf : cap;
    return `cover HL withdraw fee once from owner reserve (≤${fmtUsdc(cap)}, release ~${fmtUsdc(feeSlice)})`;
  },
  canSkip: async (ctx) => {
  ...
  execute: executeTailCoverHlFeeShortfall,
};
  ...
  if (
    ...
    throw new Error(
      `HL USDC settlement incomplete: HL spot ${fmtUsdc(fresh.hlUsdcBalance)} (target ${fmtUsdc(targetHlUsdc)}), ` +
        `adapter ${fmtUsdc(fresh.adapterUsdcErc20)}, vault ops float ${fmtUsdc(fresh.contractUsdc)}, ` +
        `strategy repay shortfall ${fmtUsdc(finalShortfall)}. ` +
